...
<main class="dash-content">
    ...
    <div class="dash-page-head">
        <div>
            ...
            <p class="dash-page-desc">{{ $agents->count() }} agents on your platform — manage verification, performance,
                and approvals.</p>
        </div>
    </div>
            ...
            <div class="col-md-6 col-lg-4">
                <div class="agent-card position-relative d-flex flex-column h-100">
                    ...
                    <div class="d-flex justify-content-between align-items-center w-100">
                        ...
                        @else
                            <div></div> {{-- Keeps 'Verified' pushed to the right when not featured --}}
                        @endif
                    ...
                    <div class="agent-card-body text-center flex-grow-1">
                        <img src="{{ asset('storage/' . $agent->user->avatar) }}" alt="{{ $agent->user->first_name }}"
                        ...
                        <div class="agent-stats-row">
                            <div><strong>{{$agent->properties->count()}}</strong><span>Listings</span></div>
                    ...
                    <div class="d-flex align-items-center gap-2 pt-2 border-top-custom">
                        ...
                        <form action="{{ route('admin.agents.toggle-feature', $agent->id) }}" method="POST"
                            class="flex-fill m-0">
                            @csrf
                            @method('PATCH')
                            <button type="submit"
                                class="dash-btn-primary w-100 py-2 {{ $agent->is_featured ? 'is-featured' : '' }}">
