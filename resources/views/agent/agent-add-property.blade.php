...
<main class="dash-content">
    ...
    <form action="{{ $isEdit ? route('property.update', $property->id) : route('properties.store') }}" method="POST"
            ...
            <div class="col-lg-12">
                <div class="dash-panel">
                    ...
                    <div class="dash-tab-pane property-tab-pane active" id="tab-basic">
                        <div class="row g-3">
                            <div class="col-md-12">
                                ...
                                <input type="text" class="dash-input @error('property_title') is-invalid @enderror"
                                    name="property_title" value="{{ old('property_title', $property->title ?? '') }}"
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="dash-select @error('property_type') is-invalid @enderror"
                                    name="property_type">
                                    @php $currentType = old('property_type', $property->type ?? ''); @endphp
                                    <option value="house" {{ $currentType == 'house' ? 'selected' : '' }}>House
                                    </option>
                                    <option value="villa" {{ $currentType == 'villa' ? 'selected' : '' }}>Villa
                                    </option>
                                    <option value="apartment" {{ $currentType == 'apartment' ? 'selected' : '' }}>
                                        Apartment</option>
                                    <option value="office" {{ $currentType == 'office' ? 'selected' : '' }}>Office
                                    </option>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="dash-select @error('property_purpose') is-invalid @enderror"
                                    name="property_purpose">
                                    @php $currentPurpose = old('property_purpose', $property->purpose ?? ''); @endphp
                                    <option value="sale" {{ $currentPurpose == 'sale' ? 'selected' : '' }}>For Sale
                                    </option>
                                    <option value="rent" {{ $currentPurpose == 'rent' ? 'selected' : '' }}>For Rent
                                    </option>
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="dash-input"
                                    value="{{ $isEdit && isset($property->agent->user) ? $property->agent->user->first_name . ' ' . $property->agent->user->last_name : Auth::user()->first_name . ' ' . Auth::user()->last_name }}"
                                    disabled>
                            </div>

                            <div class="col-12">
                                <label class="dash-form-label">Description</label>
                                <textarea class="dash-input @error('property_description') is-invalid @enderror" name="property_description"
                                    rows="5" placeholder="Describe the property...">{{ old('property_description', $property->description ?? '') }}</textarea>
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="number" step="0.01"
                                    class="dash-input @error('property_price') is-invalid @enderror"
                                    name="property_price" value="{{ old('property_price', $property->price ?? '') }}"
                                    placeholder="850000">
                    ...
                    <div class="dash-tab-pane property-tab-pane d-none" id="tab-media">
                        ...
                        <!-- Featured Image -->
                        <div class="mb-4">
                            <label class="dash-form-label">Featured Cover Image @if (!$isEdit)
                                    <span class="req">*</span>
                                @endif
                            </label>
                            <div class="dash-dropzone position-relative text-center p-4 border rounded @error('property_f_image') border-danger @enderror"
                                id="featuredDropzone">
                                ...
                                <div id="featuredPreview" class="mt-3 {{ $featuredImage ? '' : 'd-none' }}">
                                    <img src="{{ $featuredImage ? asset('storage/' . $featuredImage->image) : '' }}"
                                        id="featuredPreviewImg" class="img-thumbnail"
                                        style="max-height: 180px; object-fit: cover;">
                                </div>
                            </div>
                            @error('property_f_image')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="dash-dropzone position-relative text-center p-4 border rounded @error('property_all_images.*') border-danger @enderror"
                                id="galleryDropzone">
                    ...
                    <div class="dash-tab-pane property-tab-pane d-none" id="tab-location">
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text"
                                    class="dash-input @error('property_address') is-invalid @enderror"
                                    name="property_address"
                                    value="{{ old('property_address', $property->city->address_line ?? '') }}"
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="property_state" id="property_state_select"
                                    class="dash-select @error('property_state') is-invalid @enderror">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="property_city_id" id="property_city_select"
                                    class="dash-select @error('property_city_id') is-invalid @enderror">
                                    <option value="">Select City</option>
                                    {{ $cities }}
                                    @if (isset($cities) && count($cities) > 0)
                                        @php $selectedCityId = old('property_city_id', $property->city_id ?? ''); @endphp
                                        @foreach ($cities as $c)
                                            <option value="{{ $c->id }}" data-state="{{ $c->state }}"
                                                {{ $selectedCityId == $c->id ? 'selected' : '' }}>
                                                {{ $c->city }}
                                            </option>
                                        @endforeach
                                    @endif
                        ...
                        <div class="chip-select">
                            @if (isset($amenities) && count($amenities) > 0)
                                ...
                                @foreach ($amenities as $amenity)
                                    ...
                                    <label for="am{{ $amenity->id }}"><i class="{{ $amenity->icon }}"></i>
                                        {{ $amenity->name }}</label>
                                @endforeach
                            @else
                    ...
                    <div class="dash-tab-pane property-tab-pane d-none" id="tab-seo">
                        <div class="row g-3">
                            <div class="col-12">
                                ...
                                <input type="text" class="dash-input @error('slug') is-invalid @enderror"
                                    name="slug" value="{{ old('slug', $property->slug ?? '') }}"
