<div class="col-lg-4 col-md-6 fprop-card reveal-child">
   ...
   <i class="bi {{ $property->savedProperties->contains('user_id', auth()->id()) 
    ...
    : 'bi-heart' }}">
        ...
        <!-- Main Card Link -->
        <a href="{{ route('property.show', $property->id) }}" style="text-decoration: none;" class="d-block h-100">
            <div class="card property-card border-0 h-100">
                <div class="card-image-wrapper position-relative">
                    ...
                    <span class="badge-tag position-absolute start-0 ms-3 text-capitalize">{{ $property['type'] }}</span>
                </div>

                <div class="card-body p-4 d-flex flex-column">
                    <h5 class="card-title mb-2 text-truncate">{{ $property['title'] }}</h5>
                    <p class="card-location mb-3 d-flex align-items-center">
                        <i class="bi bi-geo-alt-fill me-2"></i>{{ $property['city']['city'] }}, {{ $property['city']['country'] }}
                    </p>
