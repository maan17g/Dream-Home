...
<body>
    <div class="dash-body">
        ...
        <aside class="dash-sidebar" id="sidebar">
            <a href="{{ route('page.index') }}" class="dash-logo"><i class="bi bi-house-door-fill"></i><span
            ...
            <ul class="dash-nav">
                <li><a href="{{ route('user.index') }}"
                        class="dash-nav-link {{ request()->routeIs('user.index') ? 'active' : '' }}"><i
                            class="bi bi-grid-1x2-fill"></i><span>Dashboard</span></a></li>
            </ul>
            <div class="dash-nav-label">My Activity</div>
            <ul class="dash-nav">
                <li><a href="{{ route('user.saved') }}"
                        class="dash-nav-link {{ request()->routeIs('user.saved') ? 'active' : '' }}"><i
                            class="bi bi-heart-fill"></i><span>Saved Properties</span><span
                            class="dash-nav-badge">{{ Auth::user()->savedProperties()->count() }}</span></a></li>
                <li><a href="{{ route('user.appointments') }}"
                        class="dash-nav-link {{ request()->routeIs('user.appointments') ? 'active' : '' }}"><i
                            class="bi bi-calendar-check-fill"></i><span>My Appointments</span><span
                            class="dash-nav-badge">{{Auth::user()->appointments->count()}}</span></a></li>
           
            </ul>
            <div class="dash-nav-label">Account</div>
            <ul class="dash-nav">
                <li><a href="{{ route('user.profile') }}"
                        class="dash-nav-link {{ request()->routeIs('user.profile') ? 'active' : '' }}"><i
                ...
                <div class="dash-topbar-right">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-end dash-dropdown-menu">
                            <a class="dropdown-item" href="{{ route('user.profile') }}"><i class="bi bi-person"></i> My
                                Profile</a>
                            <form action="{{ route('user.destroy') }}" method="GEt" class="m-0 p-0">
                                @csrf
