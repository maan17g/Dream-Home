{{-- @include('frontend.layout.header', ['title' => 'Agent Profile | Dream Home Admin']) --}}
...
<body data-theme="dark">
    ...
    <div class="dash-main">
        <div class="text-decoration-none flex-inline">
            <a href="{{ url()->previous() }}" class="page-link mt-5 ms-4 align-text-center p-2" style=" width:100px !important;">
            ...
            <div class="row g-3">
                <div class="col-lg-4">
                    <div class="dash-panel text-center mb-3">
                        <img src="{{ asset('storage/'. $agent->user->avatar) }}"
                            style="width:96px;height:96px;border-radius:50%;object-fit:cover;border:3px solid var(--primary);"
                            class="mb-2">
                        <h6 class="mb-0">{{$agent->user->first_name .' '. $agent->user->last_name}}</h6>
                        <div class="dash-row-sub mb-2">{{ $agent->user->email }}</div>
                        <span class="verified-badge" style="position:static;display:inline-flex;"><i
                                class="bi bi-patch-check-fill"></i>Verified</span>
                        <div class="rating-stars mt-2">★★★★★ <span class="text-muted-custom">{{$agent->review->sum('rating')}}</span>
                        </div>
                    </div>
                    <div class="dash-panel">
                        ...
                        <div class="d-flex justify-content-between mb-2"><span class="dash-row-sub">Phone</span><strong
                                style="font-size:.82rem;">{{$agent->user->phone?$agent->user->phone:'#'}}</strong></div>
                        <div class="d-flex justify-content-between mb-2"><span class="dash-row-sub">License
                                No.</span><strong style="font-size:.82rem;">{{$agent->license_no}}</strong></div>
                        
                        <div class="d-flex justify-content-between"><span class="dash-row-sub">Joined</span><strong
                                style="font-size:.82rem;">{{$agent->user->created_at->format('Y-m-d')}}</strong></div>
                    </div>
                </div>
                        ...
                        <div class="col-6 col-md-6">
                            <div class="stat-card">
                                ...
                                <div>
                                    ...
                                    <div class="stat-value">{{$agent->properties->count()}}</div>
                                </div>
                            </div>
                        </div>
                        <div class="col-6 col-md-6">
                            <div class="stat-card">
                                ...
                                <div>
                                    ...
                                    <div class="stat-value">{{$agent->properties->sum('views')}}</div>
                                </div>
                            </div>
                        </div>
                ...
                @forelse($properties as $property)
                    <tr>
                        <td class="d-flex align-items-center gap-2">
                            ...
                            <span class="dash-row-title">{{ $property->title }}</span>
                        </td>
                        <td>${{ number_format($property->price) }}</td>
                        <td>
                            <span class="status-pill {{ $property->verified === 'approved' ? 'success' : 'warning' }}">
                                <i class="bi bi-circle-fill"></i>
                                {{ ucfirst($property->verified) }}
                            </span>
                        </td>
    ...
    <div class="mt-3 d-flex justify-content-center">
        {{ $properties->links('pagination::bootstrap-5') }}
                    ...
                    <div class="footer-socials">
                        <a href="{{ $agent->facebook ?? '#' }}" class="footer-social-icon"><i
                                class="fab fa-facebook-f"></i></a>
                        <a href="{{ $agent->instagram ?? '#' }}" class="footer-social-icon"><i
                                class="fab fa-instagram"></i></a>
                        <a href="{{ $agent->twitter ?? '#' }}" class="footer-social-icon"><i
                                class="fab fa-x"></i></a>
                        <a href="{{ $agent->linkedin ?? '#' }}" class="footer-social-icon"><i
